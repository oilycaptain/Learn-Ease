...
/** Turn interrogatives into declarative True/False statements */

function coerceToTrueFalse(q) {
  const origQ = (q?.question || '').toString().trim();
  const ansText = resolveCorrectText(q);
  const cap = (s) => s ? s.charAt(0).toUpperCase() + s.slice(1) : s;
  const stripQ = (s) => s.replace(/\?\s*$/, '').trim();

  let statement = origQ;

  // Which/What of the following is/are X?
  let m = statement.match(/^(which|what)\s+of\s+the\s+following\s+(is|are)\s+(.+)\?$/i);
  if (m && ansText) {
    statement = `${ansText} ${m[2].toLowerCase()} ${m[3]}`;
  } else {
    // What is/are X?
    m = statement.match(/^what\s+is\s+(.+)\?$/i);
    if (m && ansText) {
      statement = `${cap(m[1])} is ${ansText}`;
    } else {
      m = statement.match(/^what\s+are\s+(.+)\?$/i);
      if (m && ansText) {
        statement = `${cap(m[1])} are ${ansText}`;
      } else {
        // What does X ... ?
        m = statement.match(/^what\s+does\s+(.+)\?$/i);
        if (m && ansText) {
          statement = `${cap(m[1])} ${ansText}`;
        } else if (ansText) {
          statement = `${cap(stripQ(statement))} ${ansText}`;
        } else {
          statement = cap(stripQ(statement));
        }
      }
    }
  }
...
/** Convert interrogatives to cloze-style fill-blank */

function coerceToFillBlank(q) {
  const orig = (q?.question || '').toString().trim();
  const ansText = resolveCorrectText(q);
  let text = orig.replace(/\?\s*$/, '');
  if (!/_{3,}/.test(text)) {
    const m = text.match(/^(Which|What)\s+of\s+the\s+following\s+(is|are)\s+(.+)\?$/i);
    if (m) {
      text = `____ ${m[2].toLowerCase()} ${m[3]}`;
    } else {
      text = text.replace(/\b(what|which)\b.*$/i, '____');
      if (!/____/.test(text)) text = `${text} ____`;
    }
  }
  if (!/[.?!]$/.test(text)) text += '.';
  return { ...q, question: text, type: 'fill-blank', options: undefined, correctAnswer: ansText };
}


/** Apply coercion for single-type quizzes */
function coerceQuizToType(quizJson, quizType) {
  if (!quizJson || !Array.isArray(quizJson.questions)) return quizJson;
  const out = { ...quizJson };
  out.questions = quizJson.questions.map(q => {
    if (quizType === 'true-false') return coerceToTrueFalse(q);
    if (quizType === 'fill-blank' || quizType === 'identification') return coerceToFillBlank(q);
    // multiple-choice -> ensure options array exists (best effort)
    if (quizType === 'multiple-choice') {
      const opts = Array.isArray(q.options) ? q.options : [];
      return { ...q, type: 'multiple-choice', options: opts.length ? opts : [q.correctAnswer, 'N/A', 'N/A', 'N/A'] };
    }
    return q;
  });
  return out;
...
function resolveCorrectText(q) {
  ...
  const raw = (q?.correctAnswer || '').toString().trim();
  const m = raw.match(/^[A-D](?:[.)]|$)/i);
  if (m && Array.isArray(q?.options) && q.options.length) {
    const idx = letters.indexOf(raw[0].toUpperCase());
    if (idx >= 0 && q.options[idx] != null) return q.options[idx].toString().trim();
  }
  return raw;
}
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    // ensure type on each question if missing
    if (Array.isArray(quiz.questions)) {
      quiz.questions = quiz.questions.map(q => ({
        ...q.toObject ? q.toObject() : q,
        type: q.type || quiz.quizType || 'multiple-choice'
      }));
    }
    res.json(quiz);
  } catch (error) {
